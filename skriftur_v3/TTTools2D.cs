...
// Tilraunagerð 2D landslagssneiðsmiður
...
namespace TTT
{
	public class TTTools2D : EditorWindow 
	{
		...
		private int edgeColliderResolution = 16; // 1 = 1 pixel resolution // 1 = 1 pixla upplausn
...
//		private Color undergroundColor = Color.black;
		...
		int slicePositionY=0; // FIXME: currently only works for 0 // FIXME: sem stendur virkar aðeins fyrir 0
...
// bool splatHasAlpha=false; // FixTextureSettings() athugar hvort áferðin inniheldur alfarás, ef já, þá verður hún notuð þegar splatmap er teiknað
		...
		void OnGUI () 
		{
			...
			*/
			...
			GUILayout.BeginHorizontal ();
			EditorGUILayout.PrefixLabel ("Terrain Textures (R G B)"); // U = underground // U = neðanjarðar
			GUILayout.FlexibleSpace ();
...
//			TTT.TerrainTools.FixTextureSettings(sourceSplatMap);
			...
			int targetHeight = 128; // TODO: allow adjusting this // TODO: leyfa að stilla þetta
			...
			for (int x = 0; x < targetWidth; x++)
			{
				...
				{
					int textureScale = 8; // TODO: allow adjusting this // TODO: leyfa að stilla þetta
					...
					Color tex4c = splatTextures[3].GetPixel(x,yPointScaled3); // underground // neðanjarðar
					...
					// við erum í landslaginu
					if (y<height*targetHeight)
					{
						c = tex4c; 
					}

					if (y>height*targetHeight-surfaceDepth && y<=height*targetHeight) // we are at surface // við erum á yfirborðinu 
					{
			...
			// stillt sem sprite í innflutningsstillingum
		...
		// endurvarpa gildi frá einu sviði til annars
